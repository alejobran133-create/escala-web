...
(async () => {
  ...
  try {
    ...
    console.log(JSON.stringify({ label: 'interaction-desktop', autoRotation, pointerTilt, midSpin, clickTurn, keyboardTurn }));
    if (!autoRotation || !pointerTilt || !clickTurn || !keyboardTurn || midSpin === 'none') throw new Error('Desktop interaction failed');
